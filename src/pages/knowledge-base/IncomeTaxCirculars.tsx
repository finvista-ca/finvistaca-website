...
import { incomeTaxCircularsData, type IncomeTaxCircular, relatedActs } from '../../data/incomeTaxCircularsData';
import './IncomeTaxCirculars.css';
...
export const IncomeTaxCirculars: React.FC = () => {
  ...
  const availableYears = useMemo(() => {
    const years = Array.from(new Set(incomeTaxCircularsData.map(c => c.year)));
    return years.sort((a, b) => b - a);
  }, []);

  const categories = useMemo(() => {
    const cats = Array.from(new Set(incomeTaxCircularsData.map(c => c.category)));
    return cats.sort();
  }, []);

  // Filter Circulars
  const filteredCirculars = useMemo(() => {
    let result = incomeTaxCircularsData;

    // Search
    if (debouncedQuery) {
      const query = debouncedQuery.toLowerCase();
      result = result.filter(circular => {
        // Exact circular number gets highest priority conceptually, but here we just filter
        return (
  ...
  const selectedCircular = useMemo(() => {
    return incomeTaxCircularsData.find(c => c.id === selectedCircularId) || null;
  }, [selectedCircularId]);
  ...
  return (
    <div className="income-tax-circulars-page">
      <InternalPageHero 
        title="Income Tax Circulars"
        description="Browse important CBDT circulars, clarifications and instructions relating to income-tax administration and compliance."
        breadcrumbs={[
          { label: 'Home', path: '/' },
          { label: 'Knowledge Base', path: '/knowledge-base' },
          { label: 'Income Tax Circulars' }
        ]}
      />
      ...
      <div className="container" style={{ marginTop: '0' }}>
        {/* AT A GLANCE */}
        <KnowledgeBaseHeader
          infoTitle="Important CBDT circulars, clarifications and instructions relating to income-tax administration and compliance."
          infoIcon={<Book className="icon" size={24} />}
          infoGrid={[
            { label: 'Circulars Indexed', value: incomeTaxCircularsData.length },
          ...
          statCards={[
            ...
            { value: incomeTaxCircularsData.length, label: 'Circulars Indexed' },
            { value: 'Searchable', label: 'Circular Directory' },
            { value: 'Verified', label: 'Official Sources' }
          ]}
        />
            ...
            {featuredCirculars.map((circular) => (
              <div key={`feat-${circular.id}`} className="circular-card" onClick={() => openCircular(circular.id)}>
                ...
                <div className="circular-tags">
                  ...
                  {circular.applicableAct && <span className="circular-tag act">{circular.applicableAct}</span>}
                </div>
      ...
      <div ref={directoryRef} className="circulars-directory-section">
        <h2 style={{ fontSize: '1.8rem', color: 'white', marginBottom: '1.5rem' }}>CBDT Circular Directory</h2>
        ...
        <div className="circular-search-filter-container">
          <div className="circular-search-wrapper">
            ...
            <input
              type="text"
              placeholder="Search circular number, topic, section or keyword... (e.g. 06/2024, TDS)"
              value={searchQuery}
              onChange={(e) => {
      ...
      <div className="official-reference-card" style={{ marginTop: '4rem', padding: '2rem', background: 'var(--glass-bg)', border: '1px solid rgba(201, 160, 80, 0.3)', borderRadius: '12px', borderLeft: '4px solid var(--brand-gold)' }}>
        <h3 style={{ color: 'white', display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1rem' }}>
          <Book size={24} color="var(--brand-gold)" /> Official CBDT Circulars
        </h3>
        <p style={{ color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: '1.5rem' }}>
          Tax laws and administrative guidance are subject to amendments, withdrawals, supersession and subsequent circulars. Verify the applicable and current position from the official Income Tax Department / CBDT source before relying on any circular.
        </p>
        <a 
        ...
        >
          View Official Circular Database <ExternalLink size={16} />
        </a>
      </div>
          ...
          <div className="circular-modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="circular-modal-body">
              ...
              <div className="circular-modal-meta-grid">
                ...
                {selectedCircular.applicableAct && (
                  <div className="circular-modal-meta-item">
                    <span className="circular-modal-meta-label">Applicable Act</span>
                    <span className="circular-modal-meta-value">{selectedCircular.applicableAct}</span>
                  </div>
                )}
